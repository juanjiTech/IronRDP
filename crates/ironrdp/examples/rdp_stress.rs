...
//! the decoded framebuffer. No browser, no WASM, no Termium: everything here talks
//! to `ironrdp-session` directly, so a failure points at the protocol/decode path
//! rather than at canvas plumbing.
//!
...
//!     --host 192.168.28.181 -u homeb --rounds 10 --out-dir /tmp/rdp-stress
//! ```
//!
...
fn main() -> anyhow::Result<()> {
    ...
    if failures > 0 {
        anyhow::bail!("{failures} step(s) exceeded the black-tile threshold");
    }
...
impl Measurement {
    fn failed(&self, config: &Config) -> bool {
        self.black_tile_pct > config.threshold
    }
}
...
fn settle(
    ...
    loop {
        ...
        let healthy = measured.black_tile_pct <= config.threshold
            && measured.stale_tile_pct.is_none_or(|stale| stale <= config.threshold);
...
fn save_png(image: &DecodedImage, dir: &PathBuf, step: &str) -> anyhow::Result<()> {
