...
export class RemoteDesktopService {
    ...
    private releaseMouseButtons() {
        ...
        // Force-release all three; browser button state is unreliable once the pointer left.
        const releases = buttonsMap.map(([, buttonId]) => this.module.DeviceEvent.mouseButtonReleased(buttonId));
        this.doTransactionFromDeviceEvents(releases);
    }
